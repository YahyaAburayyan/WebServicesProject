...
# maps each status to the timestamp field we update when entering that state
TIMESTAMP_FIELD = {
...
def next_sequence(name: str) -> int:
    # atomic increment so two requests never get the same number
...
def to_public(doc: dict | None) -> dict | None:
    # mongodb stores _id as ObjectId which can't be serialized to JSON,
    # so we convert it to a plain string called "id"
...
def set_state(app_id: str, target: str, extra: dict | None = None) -> None:
    ...
    # stamp the specific timestamp for this state if it has one
    ts_field = TIMESTAMP_FIELD.get(target)
    if ts_field:
        changes[f"timestamps.{ts_field}"] = now()
